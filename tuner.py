...
def tuner(beys=True, func='', dir='./', args='', compile_args='', optTarget=1, Par=[], output_binary = "tuned", flto=0, optPass='-O3'):
    ...
    Par_Val=Par[6]
    if beys:
        #optimize_compiler_parameters(numIter=numIter, output_binary=output_binary,  numTest=numTest, thresholds=[evalInit['GCUPS']], compile_args='')
        print(f"Result calculated for:\nnumIter:{numIter},\tnumTest: {numTest},\tflto: {flto},\toptTarget: {optTarget},\toptPass: {optPass}")
    else:
